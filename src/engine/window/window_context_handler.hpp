...
public:
         ...
         , m_render_context(nullptr)
    {
        ...
        m_render_context->setRenderSize(static_cast<IVec2>(window_size));
        // Initialize base viewport control
        registerDefaultCallbacks(true);
    }

    void setMouseCursorVisible(bool b)
    {
    ...
    void setMousePosition(sf::Vector2i position)
    {
        ...
        m_render_context->m_viewport_handler.setMousePosition(m_event_manager.getFloatMousePosition());
    }

    [[nodiscard]]
    Vec2 getWindowPosition(Vec2 position) const
    {
        return m_render_context->m_viewport_handler.getTransform() * position;
    }
